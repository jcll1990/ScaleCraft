...
const PlayScalesIntermediate = ({
  user,
  navigation,
  soundFiles,
  soundset,
  noteSet,
  scalesIntermediateTime,
  scalesIntermediatePlays,
    ...
    function NotmainButton(a) {
      ...
      if (!chosenNote1) {
        ...
        {      
          setResult("Correct");
          if (user) {
            setScalesIntermediateTime(prevState => prevState + timer);
            setScalesIntermediatePlays(prevState => prevState + 1);
            save(parseFloat(scalesIntermediateTime) + timer, parseFloat(scalesIntermediatePlays) + 1);
          }
          stopTimer();
          setflag(false)
      ...
      const save = async (updatedNoteTime, updatedNotePlays) => {
        try {
          await AsyncStorage.setItem("scalesIntermediateTime", updatedNoteTime.toString());
          await AsyncStorage.setItem("scalesIntermediatePlays", updatedNotePlays.toString());
        } catch (err) {
          alert(err);
        }
      };
  ...
  <View style={styles.main}>
    ...
    <View style={styles.mainDisplay}>
      ...
      <Text style={styles.timer}>Natural scales{'\n'} {timer > 0 ? `Timer: ${timer}` : null}</Text>
...
export default PlayScalesIntermediate;
