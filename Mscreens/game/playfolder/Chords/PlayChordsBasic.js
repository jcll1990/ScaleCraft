...
const PlayChordsBasic = ({
  ...
  const TheMainButton = () => {
    ...
    const newRandom2 = Math.floor(randomDecimal2 * 3) + 1;
    ...
    } else if (newRandom2===3){
      if (newRandom1 + 3 > 12) 
        {
      setGameNote2(newRandom1 + 3 - 12)
      setGameNote3(newRandom1 + 3 + 3 - 12)
        }
      else if (newRandom1 + 3 + 3 > 12){
        setGameNote2(newRandom1 + 3 )
        setGameNote3(newRandom1 + 3 + 3 - 12)
      } else {
        setGameNote2(newRandom1 + 3 )
        setGameNote3(newRandom1 + 3 + 3)
      }
    }
    startTimer();
  ...
  const generateRandomNumbers = () => {
    ...
    useEffect(() => {
      ...
      if (basicChord === 1) {
        setDisplayGameChord("Maj");
      } else if (basicChord === 2) {
        setDisplayGameChord("Min");
      } else if (basicChord === 3) {
        setDisplayGameChord("Dim");
      } 
    ...
    function NotmainButton(a) {
      ...
      if (!chosenNote1) {
        ...
        if (chosenNote1 === gameNote && chosenNote2 === gameNote2 && a === gameNote3 && flag === true) {
        
          setResult("Correct");
        if (user) {
          setChordBasicTime(prevState => prevState + timer);
          setChordBasicPlays(prevState => prevState + 1);
          save(parseFloat(chordBasicTime) + timer, parseFloat(chordBasicPlays) + 1);
        }
        stopTimer();
        setflag(false)
    ...
    const save = async (updatedNoteTime, updatedNotePlays) => {
      try {
        await AsyncStorage.setItem("chordBasicTime", updatedNoteTime.toString());
        await AsyncStorage.setItem("chordBasicPlays", updatedNotePlays.toString());
      } catch (err) {
        alert(err);
      }
    };
  ...
  <View style={styles.main}>
    ...
    <View style={styles.mainDisplay}>
      <View style={styles.timerDisplay}>
        <Text style={styles.timer}>Triad chords{'\n'} {timer > 0 ? `Timer: ${timer}` : null}</Text>
      </View>
...
export default PlayChordsBasic;
