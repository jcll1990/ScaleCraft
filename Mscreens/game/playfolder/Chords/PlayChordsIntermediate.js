...
const PlayChordsIntermediate= ({
  user,
  navigation,
  soundFiles,
  soundset,
  noteSet,
  chordIntermediateTime,
  chordIntermediatePlays,
  ...
  const [interChord, setInterChord] = useState();
  // 1 = Major Seventh Chord (Maj7) +4 +3 +4
  // 2 = Minor Seventh Chord (m7) +3 +4 +3
  // 3 = Dominant Seventh Chord (7) +4 +3 +3
  // 4 = Half-Diminished Seventh Chord (m7b5) +3 +4 +3
  const [displayGameChord, setDisplayGameChord] = useState();
  ...
  const TheMainButton = () => {
    ...
    const newRandom2 = Math.floor(randomDecimal2 * 4) + 1;
  
    setInterChord(newRandom2);
    setGameNote(newRandom1);


    if(newRandom2===1){ // 1 = Major Seventh Chord (Maj7) +4 +3 +4

      if (newRandom1 + 4 > 12) 
        {
      setGameNote2(newRandom1 + 4 -12)
      setGameNote3(newRandom1 + 4 + 3 - 12)
      setGameNote4(newRandom1 + 4 + 3 + 4 -12)
        }
      else if (newRandom1 + 4 + 3 > 12){
        setGameNote2(newRandom1 + 4 )
        setGameNote3(newRandom1 + 4 + 3 - 12)
        setGameNote4(newRandom1 + 4 + 3 + 4 -12)

      } else if( newRandom1 + 4 + 3 + 4 > 12) {
        setGameNote2(newRandom1 + 4)
        setGameNote3(newRandom1 + 4 + 3)
        setGameNote4(newRandom1  + 4 + 3  +4 -12)
      } else {
        setGameNote2(newRandom1 + 4 )
        setGameNote3(newRandom1 + 4 + 3)
        setGameNote4(newRandom1  + 4 + 3 +4)
      }

    } else if (newRandom2===2){  // 2 = Minor Seventh Chord (m7) +3 +4 +3
      if (newRandom1 +3 > 12) 
        {
      setGameNote2(newRandom1 +3 - 12)
      setGameNote3(newRandom1 +3 +4 - 12)
      setGameNote4(newRandom1 +3 +4 +3 -12)
        }
      else if (newRandom1 +3 +4 > 12){
        setGameNote2(newRandom1 +3)
        setGameNote3(newRandom1 +3 +4 - 12)
        setGameNote4(newRandom1 +3 +4 +3 -12)

      } else if( newRandom1 +3 +4 +3> 12) {
        setGameNote2(newRandom1 +3)
        setGameNote3(newRandom1 +3 +4)
        setGameNote4(newRandom1  +3 +4 +3 -12)
      } else {
        setGameNote2(newRandom1 +3)
        setGameNote3(newRandom1 +3 +4)
        setGameNote4(newRandom1  +3 +4 +3)
      }
    } else if (newRandom2===3){  // 3 = Dominant Seventh Chord (7) +4 +3 +3
      if (newRandom1 +4> 12) 
        {
      setGameNote2(newRandom1 +4 - 12)
      setGameNote3(newRandom1 +4 +3 - 12)
      setGameNote4(newRandom1 +4 +3 +3 -12)
        }
      else if (newRandom1 +4 +3 > 12){
        setGameNote2(newRandom1 +4 )
        setGameNote3(newRandom1 +4 +3- 12)
        ...
        setGameNote2(newRandom1 +4)
        setGameNote3(newRandom1 +4 +3)
        setGameNote4(newRandom1  +4 +3 +3)
      }
    } else if (newRandom2===4){ // 4 = Half-Diminished Seventh Chord (m7b5) +3 +4 +3
      if (newRandom1 +3> 12) 
        {
      setGameNote2(newRandom1 +3 - 12)
      setGameNote3(newRandom1 +3 +4 - 12)
    ...
    useEffect(() => {
      ...
      if (interChord === 1) {
        setDisplayGameChord("Maj7");
      } else if (interChord === 2) {
        setDisplayGameChord("m7");
      } else if (interChord === 3) {
        setDisplayGameChord("7");
      } else if (interChord === 4) {
        setDisplayGameChord("m7b5");
      } 
    ...
    function NotmainButton(a) {
      ...
      if (!chosenNote1) {
        ...
        if (chosenNote1 === gameNote && chosenNote2 === gameNote2 && chosenNote3 === gameNote3 && a === gameNote4 && flag === true) 
        {    
          if (user) {
            setChordIntermediateTime(prevState => prevState + timer);
            setChordIntermediatePlays(prevState => prevState + 1);
            save(parseFloat(chordIntermediateTime) + timer, parseFloat(chordIntermediatePlays) + 1);
          }
          stopTimer();
          setflag(false)
      ...
      const save = async (updatedNoteTime, updatedNotePlays) => {
        try {
          await AsyncStorage.setItem("chordIntermediateTime", updatedNoteTime.toString());
          await AsyncStorage.setItem("chordIntermediatePlays", updatedNotePlays.toString());
        } catch (err) {
          alert(err);
        }
      };
  ...
  <View style={styles.main}>
    ...
    <View style={styles.mainDisplay}>
      <View style={styles.timerDisplay}>
        <Text style={styles.timer}>Seventh chords{'\n'} {timer > 0 ? `Timer: ${timer}` : null}</Text>
      </View>
...
export default PlayChordsIntermediate;
